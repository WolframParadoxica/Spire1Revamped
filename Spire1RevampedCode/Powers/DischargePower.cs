...
public sealed class DischargePower : Spire1RevampedPower
{
  ...
  {
    DischargePower dischargePower = this;
    if (player != dischargePower.Owner.Player)
      return;
    await PowerCmd.Decrement((PowerModel) dischargePower);
  }
}
...
public static class LightningOrb_Passive_Patch
{
  ...
  {
    if (!__instance.Owner.Creature.HasPower<DischargePower>())
      return;
