    ...
    TargetType.AnyEnemy)
{
        ...
        CardPlay cardPlay)
    {
        ...
        AttackCommand attackCommand = await DamageCmd.Attack(card.DynamicVars.Damage.BaseValue).WithHitCount(2).FromCard((CardModel) card, cardPlay).Targeting(cardPlay.Target).WithHitFx("vfx/vfx_attack_slash", tmpSfx: "event:/sfx/characters/silent/silent_dagger_spray").Execute(choiceContext);
